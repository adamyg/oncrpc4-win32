...
#if defined(RPCBIND_SERVICE)

#define LIBRPC_NO_SOCKET_MAPPINGS
...
static const char *	appname = "rpcbind";
...
main(int argc, const char **argv)
{
	...
	// Several options are duplicates of rpcbind's options, which need to be handled prior to service initialisation.
...
help(void)
{
	printf(LIBRPC_PACKAGE " - " LIBRPC_PACKAGE_NAME " " LIBRPC_VERSION ", service options\n\n");

	printf("NAME:\n\n");
	printf("rpcbind service\n\n");

	printf("SYNOPSIS:\n\n");
	printf("rpcbind [options] <command> [configuration]\n\n");
	...
	for (unsigned i = 0; i < _countof(rpcbind_usage); ++i) {
		printf("%s\n", rpcbind_usage[i]);
	}
...
license(void)
{
	printf(LIBRPC_PACKAGE " - " LIBRPC_PACKAGE_NAME " " LIBRPC_VERSION "\n\n");

	printf("Copyright (C) 2022 Adam Young, All rights reserved.\n");
	printf("Licensed under GNU AGPL version 3.0.\n");
...
version(void)
{
	printf("%s %s [Build %s %s]\n", LIBRPC_PACKAGE, LIBRPC_VERSION, LIBRPC_BUILD_NUMBER, LIBRPC_BUILD_DATE);
	exit(3);
}

#endif /*RPCBIND_SERVICE*/
