...
static CLIENT	*clnt_com_create(struct sockaddr_in *, rpcprog_t, rpcvers_t, int *, const char *);
...
static CLIENT	*clnt_addr_create(const char *, const struct netconfig *, rpcprog_t, rpcvers_t);
static CLIENT   *clnt_rpcbind_create(const char *, rpcvers_t, struct netbuf **);
static CLIENT   *getclnthandle(const char *, const struct netconfig *, rpcvers_t, struct netbuf **);
...
#ifdef PORTMAP
static CLIENT *
clnt_com_create(struct sockaddr_in *addr, rpcprog_t prog, rpcvers_t vers, int *fdp, const char *trans)
{
	CLIENT *clnt;
...
ip_ping(u_short portnum, const char *trans, int argc, char **argv)
{
	...
	rpc_stat = CLNT_CALL(client, NULLPROC, (xdrproc_t)xdr_void, NULL, (xdrproc_t)xdr_void, NULL, to);
	if (argc != 2) {
...
)
{
	...
	struct sockaddr *sa = (struct sockaddr *)who->buf;
        socklen_t sa_len = SOCKLEN_SOCKADDR_PTR(sa);

	if (getnameinfo(sa, sa_len, hostbuf, NI_MAXHOST, NULL, 0, 0)) {
		hostname = unknown;
	} else {
		hostname = hostbuf;
	}
 ...
 */
static CLIENT *
clnt_addr_create(const char *address, const struct netconfig *nconf, rpcprog_t prog, rpcvers_t vers)
{
	CLIENT *client;
...
static CLIENT *
clnt_rpcbind_create(const char *host, rpcvers_t rpcbversnum, struct netbuf **targaddr)
{
...
static CLIENT*
getclnthandle(const char *host, const struct netconfig *nconf, rpcvers_t rpcbversnum, struct netbuf **targaddr)
{
	struct netbuf addr;
	struct addrinfo hints, *res;
	CLIENT *client = NULL;
	...
	client = clnt_tli_create(RPC_ANYFD, nconf, &addr, RPCBPROG, rpcbversnum, 0, 0);
	if (client) {
		if (targaddr != NULL) {
			*targaddr = malloc(sizeof(**targaddr));
			if (*targaddr != NULL) {
				...
				if ((*targaddr)->buf != NULL) {
					(void)memcpy((*targaddr)->buf, addr.buf, addr.len);
				}
			}
		}
	} else {
...
print_rmtcallstat(int rtype, const rpcb_stat *infp)
{
	...
	if (rtype == RPCBVERS_4_STAT)
		(void)printf("prog\t\tvers\tproc\tnetid\tindirect success failure\n");
	else
	...
	for (pr = infp->rmtinfo; pr; pr = pr->next) {
		...
		(void)printf("%d\t%d\t%s\t", pr->vers, pr->proc, pr->netid);
		if (rtype == RPCBVERS_4_STAT)
